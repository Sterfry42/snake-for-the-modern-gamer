...
describe('SnakeGame thermal body state', () => {
  it('does not accumulate heat exposure with the Hell escape immunity', () => {
    ...
    expect((game as unknown as { tickTemperatureState(): boolean }).tickTemperatureState()).toBe(
      false,
    );
  ...
  it('uses Mosaic Coast tile exposure for direct sun, shade, and cooling', () => {
    ...
    const gameInternal = game as unknown as {
      snake: { currentRoomId: string; body: { x: number; y: number }[] };
    };
