...
export class ArcadeSnakeRenderer {
  ...
  private playBlueScreen(): void {
    ...
    this.run.isPaused = true;
    this.options.setMusicState?.('paused');
    this.options.playEffect?.('dennis');
    this.overlayText
      .setVisible(true)
      .setBackgroundColor('#24003f')
      .setColor('#d8b4fe')
      .setText('■\nFREAK DENNIS');
    this.addTimer(900, () => {
      this.options.playEffect?.('blue-screen');
      this.frame?.setFillStyle(0x0636a8, 1).setStrokeStyle(4, 0xffffff);
      this.overlayText
        ?.setBackgroundColor('#0636a8')
        .setColor('#ffffff')
        .setText(
      ...
      this.addTimer(1200, () => {
        if (!this.run) return;
        this.frame?.setFillStyle(0x050a08, 0.99).setStrokeStyle(4, 0x58d66b);
        this.overlayText?.setBackgroundColor('#07110a').setColor('#ffffff').setVisible(false);
        this.blueScreenActive = false;
        this.run.isPaused = false;
        this.run.disabledDirectionCooldownUntilTick = Math.max(
          this.run.disabledDirectionCooldownUntilTick ?? 0,
          this.run.tick + Math.ceil(8_000 / ARCADE_TICK_MS),
        );
        this.options.setMusicState?.('corrupted');
        this.render();
      });
  ...
  private playVisualGlitch(
    ...
    if (glitch === 'text') {
      const original = this.scoreText?.text ?? '';
      this.scoreText?.setText(Math.random() < 0.5 ? 'Score: NaN     Level: -1' : 'Score: 999999');
      this.addTimer(140, () => this.scoreText?.setText(original));
      return;
    }
    const color = Math.random() < 0.5 ? 0xffffff : 0x8b5cf6;
    graphics.fillStyle(color, 0.62);
    if (glitch === 'row-shift') {
      const row = Math.floor(Math.random() * ARCADE_GRID_HEIGHT);
      graphics.fillRect(
        this.gridOrigin.x + (Math.random() < 0.5 ? -8 : 8),
        this.gridOrigin.y + row * this.tileSize,
        ARCADE_GRID_WIDTH * this.tileSize,
        this.tileSize,
      );
    } else {
      graphics.fillRect(
        this.gridOrigin.x + Math.floor(Math.random() * 13) * this.tileSize,
        this.gridOrigin.y + Math.floor(Math.random() * 9) * this.tileSize,
        glitch === 'chunk-swap' ? this.tileSize * 3 : this.tileSize,
        glitch === 'chunk-swap' ? this.tileSize * 2 : this.tileSize,
      );
    }
    this.addTimer(70, () => graphics.clear());
  }
