...
export class ScrollableTabbedMenu {
  ...
  private clampActiveState(): void {
    const state = this.getActiveState();
    const items = this.getActiveItems();
    state.selectedIndex = clamp(state.selectedIndex, 0, Math.max(0, items.length - 1));
    state.scrollIndex = clamp(state.scrollIndex, 0, Math.max(0, items.length - this.visibleItems));
  }
}
