...
export class ForestOperations {
  ...
  placeDenseForestThresholds(
    ...
    for (const neighbor of diagonalNeighbors) {
      if (
        this.biomeMap.getBiomeForRoomId(`${roomX + neighbor.dx},${roomY + neighbor.dy},${roomZ}`)
          .id !== 'elderwood-maze'
      ) {
        continue;
      }
  ...
  private resolveDenseForestPlan(
    ...
    for (const side of this.forestSides()) {
      const neighbor = this.forestNeighbor(roomX, roomY, side);
      const neighborBiome = this.biomeMap.getBiomeForRoomId(
        `${neighbor.x},${neighbor.y},${roomZ}`,
      ).id;
