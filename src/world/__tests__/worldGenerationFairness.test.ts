...
function generateNeighborhood(center: RoomCoord, radius: number): Map<string, RoomSnapshot> {
  const generator = new RoomGenerator(defaultGameConfig.world, createRng(`fairness:${roomId(center)}`));
...
function generateArea(seed: string, minX: number, maxX: number, minY: number, maxY: number): Map<string, RoomSnapshot> {
...
function edgeCellsFor(room: RoomSnapshot, grid: GridConfig, direction: Direction): Array<{ x: number; y: number }> {
...
function hasRiverSegment(room: RoomSnapshot, grid: GridConfig): boolean {
  ...
  const westEastWater = Array.from({ length: grid.rows }, (_, y) => y)
    .some((y) => room.layout[y]?.[0] === "~" && room.layout[y]?.[grid.cols - 1] === "~");
  const northSouthWater = Array.from({ length: grid.cols }, (_, x) => x)
    .some((x) => room.layout[0]?.[x] === "~" && room.layout[grid.rows - 1]?.[x] === "~");
...
function sharedWallRun(
  ...
  if (!first || !second || first.biomeId === "elderwood-maze" || second.biomeId === "elderwood-maze") {
    return 0;
  }
  ...
  for (let i = 0; i < length; i += 1) {
    const firstTile = direction === "east"
      ? first.layout[i]?.[grid.cols - 1]
      : first.layout[grid.rows - 1]?.[i];
    const secondTile = direction === "east"
      ? second.layout[i]?.[0]
      : second.layout[0]?.[i];
...
describe("world generation fairness", () => {
  ...
  it("places local structures without relying on entrance run-up cleanup", () => {
    ...
    const village = tryPlaceVillage(villageLayout, defaultGameConfig.grid, createRng("fair-village"), "verdigris-basin", {
      forbiddenCells,
      margin: HARD_RUNUP_TILES,
    });
  ...
  it("keeps rivers out of the starting neighborhood", () => {
    ...
    const wateryRooms = [...rooms.values()].filter((room) => room.layout.some((row) => row.includes("~")));
  ...
  it("spawns optional structures and cross-room features in a broad generated area", () => {
    ...
    const rivers = [...rooms.values()].filter((room) => hasRiverSegment(room, defaultGameConfig.grid));

    let crossRoomBoundaryWalls = 0;
    for (let y = -18; y <= 18; y += 1) {
      for (let x = -18; x <= 18; x += 1) {
        const current = rooms.get(`${x},${y},0`);
        if (sharedWallRun(current, rooms.get(`${x + 1},${y},0`), "east", defaultGameConfig.grid) >= 4) {
          crossRoomBoundaryWalls += 1;
        }
        if (sharedWallRun(current, rooms.get(`${x},${y + 1},0`), "south", defaultGameConfig.grid) >= 4) {
          crossRoomBoundaryWalls += 1;
        }
      }
    }
  ...
  it("does not flatten the full starting neighborhood into open clearings", () => {
    ...
    const openClearingRooms = [...rooms.values()].filter((room) => room.archetypeId === "open-clearing");
  ...
  it("uses open clearings as structure-friendly rooms instead of empty rooms", () => {
    ...
    const openClearings = [...rooms.values()]
      .filter((room) => room.archetypeId === "open-clearing" && room.biomeId !== "sunken-ocean" && room.biomeId !== "elderwood-maze");
