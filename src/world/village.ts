...
function drawHouse(layout: string[][], left: number, top: number, width: number, height: number, doorSide: "south" | "north") {
...
export function tryPlaceVillage(
  ...
  const residents = residentSpots.slice(0, 3).map((spot, index) => {
    const name = VILLAGER_NAMES[(Math.floor(rng() * VILLAGER_NAMES.length) + index + 1) % VILLAGER_NAMES.length];
    const residentPortrait = VILLAGER_PORTRAITS[(Math.floor(rng() * VILLAGER_PORTRAITS.length) + index) % VILLAGER_PORTRAITS.length];
    return {
