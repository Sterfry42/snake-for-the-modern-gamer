...
describe('achievement AP mapping', () => {
  ...
  it('appends restored achievements without shifting the existing AP range', () => {
    ...
    expect(achievementLocationId(ACHIEVEMENT_DEFINITIONS, 'streaming.followers20')).toBe(912001070);
    expect(achievementLocationId(ACHIEVEMENT_DEFINITIONS, 'maneuvers.firstLearned')).toBe(
      912001071,
    );
    expect(achievementLocationId(ACHIEVEMENT_DEFINITIONS, 'maneuvers.allLearned')).toBe(912001072);
    expect(achievementLocationId(ACHIEVEMENT_DEFINITIONS, 'vehicles.runOverEnemy')).toBe(912001073);
    expect(() => achievementLocationId(ACHIEVEMENT_DEFINITIONS, 'archaeology.firstDig')).toThrow(
      /not AP-enabled/,
    );
    expect(achievementLocationId(ACHIEVEMENT_DEFINITIONS, 'divine.snakeOutOfHell')).toBe(912001074);
  });
});
